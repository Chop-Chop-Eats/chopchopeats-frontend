...
class WalletServices {
  ...
  static Future<List<RecentWalletHistoryItem>> getRecentWalletHistory() async {
    ...
    Logger.info(
        'WalletServices', 'getRecentWalletHistory response: ${response.data}');
    if (response.data is List) {
      final List<dynamic> dataList = response.data as List<dynamic>;
      return dataList
          .map((e) =>
              RecentWalletHistoryItem.fromJson(e as Map<String, dynamic>))
          .toList();
    } else {
  ...
  static Future<List<AllWalletHistoryItem>> getAllWalletHistory() async {
    ...
    Logger.info(
        'WalletServices', 'getAllWalletHistory response: ${response.data}');
    if (response.data is List) {
      final List<dynamic> dataList = response.data as List<dynamic>;
      return dataList
          .map((e) => AllWalletHistoryItem.fromJson(e as Map<String, dynamic>))
          .toList();
    } else {
      throw Exception('API 返回的数据格式不正确，期望 List 类型');
    }
  ...
  // 创建充值订单
  static Future<String> createRechargeCardOrder(
      RechargeCardOrderParams params) async {
    final response = await ApiClient().post(
      ApiPaths.createRechargeCardOrderApi,
      data: params.toJson(),
    );
    Logger.info(
        'WalletServices', 'createRechargeCardOrder response: ${response.data}');
  ...
  static Future<List<RechargeCardItem>> getRechargeCardList() async {
    ...
    Logger.info(
        'WalletServices', 'getRechargeCardList response: ${response.data}');
    if (response.data is List) {
      final List<dynamic> dataList = response.data as List<dynamic>;
      return dataList
          .map((e) => RechargeCardItem.fromJson(e as Map<String, dynamic>))
          .toList();
    } else {
      throw Exception('API 返回的数据格式不正确，期望 List 类型');
    }
  ...
  // 创建充值订单的 Stripe PaymentIntent
  static Future<Map<String, dynamic>> createRechargeSPI(
    String orderNo,
    ...
    Logger.info(
        'WalletServices', 'createRechargeSPI response: ${response.data}');
